...
public class Trainer extends JsonFile {
    ...
    private int aiLevel;
    private boolean randomizeLeads;
    ...
    // the configured team, in the order the editor shows and stores it.
    ...
    public List<BattlePokemon> rollBattleTeam() {
        List<BattlePokemon> battleTeam = getBattleTeam();
        if (randomizeLeads) Collections.shuffle(battleTeam);
        return battleTeam;
    }
    ...
    public boolean isDoubles() {
        return "doubles".equals(battleType);
    }
    ...
    public boolean isRandomizeLeads() {
        return randomizeLeads;
    }

    public void setRandomizeLeads(boolean randomizeLeads) {
        this.randomizeLeads = randomizeLeads;
        save();
    }
    ...
    protected void loadFromJson(JsonElement jsonElement) {
        ...
        if (jsonObject.has(TRAINER_BATTLE_TYPE)) {
            battleType = jsonObject.get(TRAINER_BATTLE_TYPE).getAsString();
        }
        if (jsonObject.has(TRAINER_AI_LEVEL)) {
            aiLevel = jsonObject.get(TRAINER_AI_LEVEL).getAsInt();
        }
        if (jsonObject.has(TRAINER_RANDOMIZE_LEADS)) {
            randomizeLeads = jsonObject.get(TRAINER_RANDOMIZE_LEADS).getAsBoolean();
        }
    }
    ...
    public JsonElement toJson() {
        ...
        jsonObject.addProperty(TRAINER_BATTLE_TYPE, battleType);
        jsonObject.addProperty(TRAINER_AI_LEVEL, aiLevel);
        jsonObject.addProperty(TRAINER_RANDOMIZE_LEADS, randomizeLeads);
