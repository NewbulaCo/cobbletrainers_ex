...
public class SetupMenu extends Menu<SetupMenu> {
    ...
    private static final String PARTY_MAX_LEVEL = "party_max_level";
    private static final String DEFEAT_REQS = "defeat_reqs";
    ...
    protected void registerViewFactories(Map<String, ViewFactory<SetupMenu>> viewFactories) {
        ...
        viewFactories.put(TRAINER, new ViewFactoryBuilder<SetupMenu>()
            ...
            .withComponent(new ComponentBuilder<SetupMenu>(1, 4, Items.COMPARATOR)
                .withName("Trainer Settings")
        ...
        viewFactories.put(SETTINGS, new ViewFactoryBuilder<SetupMenu>()
            .returnsTo(TRAINER)
            .withComponent(() -> new ComponentBuilder<SetupMenu>(4, 0, POKE_BALL)
                .withName(selectedTrainer.getName())
                .build()
            )
            .withComponent(() -> new ComponentBuilder<SetupMenu>(1, 1, Items.IRON_SWORD)
                .withName("Battle Type: " + (selectedTrainer.isDoubles() ? "Doubles" : "Singles"))
                .withLore("Click to toggle singles/doubles")
                .withAction(menu -> selectedTrainer.setBattleType(
                    selectedTrainer.isDoubles() ? "singles" : "doubles"
                ))
                .refreshes()
                .build()
            )
            .withComponent(() -> {
                int level = selectedTrainer.getAiLevel();
                return new ComponentBuilder<SetupMenu>(3, 1, Items.ENCHANTED_BOOK)
                    .withName("AI Level: " + (level < 0 ? "Inherit global" : String.valueOf(level)))
                    .withLore("0-11 sets difficulty; -1 uses the server default")
                    .navigatesTo(AI_LEVEL)
                    .build();
            })
            .withComponent(() -> new ComponentBuilder<SetupMenu>(5, 1, Items.ENDER_PEARL)
                .withName("Randomize Leads: " +
                    (selectedTrainer.isRandomizeLeads() ? GREEN + "ON" : RED + "OFF"))
                .withLore("Shuffle the send-out order each battle")
                .withAction(menu -> selectedTrainer.setRandomizeLeads(!selectedTrainer.isRandomizeLeads()))
                .refreshes()
                .build()
            )
            .withComponent(() -> new ComponentBuilder<SetupMenu>(1, 3, Items.NETHER_STAR)
                .withName("Beatable Once: " +
                    (selectedTrainer.canOnlyBeatOnce() ? GREEN + "ON" : RED + "OFF"))
                .withLore("Each player may win only once")
                .withAction(menu -> selectedTrainer.setCanOnlyBeatOnce(!selectedTrainer.canOnlyBeatOnce()))
                .refreshes()
                .build()
            )
            .withComponent(() -> new ComponentBuilder<SetupMenu>(2, 3, Items.CLOCK)
                .withName("Cooldown: " + selectedTrainer.getCooldownSeconds() + "s")
                .withLore("Wait between rematches")
                .navigatesTo(COOLDOWN)
                .build()
            )
            .withComponent(() -> new ComponentBuilder<SetupMenu>(3, 3, WISE_GLASSES)
                .withName("Party Max Level: " + selectedTrainer.getPartyMaximumLevel())
                .withLore("Caps the player's team level in this battle")
                .navigatesTo(PARTY_MAX_LEVEL)
                .build()
            )
            .withComponent(() -> new ComponentBuilder<SetupMenu>(4, 3, POKE_BALL)
                .withName("Defeat Requirements: " + selectedTrainer.getDefeatRequiredTrainers().size())
                .withLore("Trainers a player must beat first")
                .navigatesTo(DEFEAT_REQS)
                .build()
            )
            .withComponent(() -> {
                boolean set = !selectedTrainer.getWinCommand().isEmpty();
                return new ComponentBuilder<SetupMenu>(5, 3, Items.WRITABLE_BOOK)
                    .withName("Win Command: " + (set ? GREEN + selectedTrainer.getWinCommand() : RED + "None"))
                    .withLore("Set via /trainers setwincommand")
                    .build();
            })
            .withComponent(() -> {
                boolean set = !selectedTrainer.getLossCommand().isEmpty();
                return new ComponentBuilder<SetupMenu>(6, 3, Items.WRITABLE_BOOK)
                    .withName("Loss Command: " + (set ? GREEN + selectedTrainer.getLossCommand() : RED + "None"))
                    .withLore("Set via /trainers setlosscommand")
                    .build();
            })
            .build()
        );

        viewFactories.put(AI_LEVEL, new ViewFactoryBuilder<SetupMenu>()
            .returnsTo(SETTINGS)
            .integerEditor(
                () -> selectedTrainer.getAiLevel(),
                value -> selectedTrainer.setAiLevel(value),
                -1, 11, 1, 5,
        ...
        viewFactories.put(COOLDOWN, new ViewFactoryBuilder<SetupMenu>()
            .returnsTo(SETTINGS)
            .integerEditor(
                () -> (int) selectedTrainer.getCooldownSeconds(),
                value -> selectedTrainer.setCooldownSeconds(value),
                0, 86400, 60, 3600,
                () -> Items.CLOCK,
                () -> "Cooldown (s)"
            ).build()
        ...
        viewFactories.put(PARTY_MAX_LEVEL, new ViewFactoryBuilder<SetupMenu>()
            .returnsTo(SETTINGS)
            .integerEditor(
                () -> selectedTrainer.getPartyMaximumLevel(),
                value -> selectedTrainer.setPartyMaximumLevel(value),
                1, 100, 1, 10,
                () -> WISE_GLASSES,
                () -> "Party Max Level"
            ).build()
        ...
        // click a trainer to add/remove it as a prerequisite. the trainer being edited is skipped so
        // it can't require beating itself.
        viewFactories.put(DEFEAT_REQS, new ViewFactoryBuilder<SetupMenu>()
            .returnsTo(SETTINGS)
            .paged9x6(
                () -> CobblemonTrainers.INSTANCE.getTrainerRegistry().getAllTrainers().stream()
                    .filter(trainer -> !trainer.getName().equals(selectedTrainer.getName()))
                    .sorted(Comparator.comparing(Trainer::getName))
                    .toList(),
                trainer -> {
                    boolean required = selectedTrainer.getDefeatRequiredTrainers().contains(trainer.getName());
                    return itemStack(required ? Items.LIME_STAINED_GLASS_PANE : POKE_BALL)
                        .withName((required ? GREEN : RED) + trainer.getName());
                },
                (menu, trainer) -> {
                    if (!selectedTrainer.removeDefeatRequirement(trainer.getName())) {
                        selectedTrainer.addDefeatRequirement(trainer.getName());
                    }
                    menu.refresh();
                }
            )
