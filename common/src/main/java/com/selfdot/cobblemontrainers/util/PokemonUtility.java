...
public class PokemonUtility {
    ...
    private static BattleStartResult createTrainerBattle(
        ...
        // a per-trainer aiLevel (0-11) selects a score-AI tier for this fight; -1 inherits the
        // global config AI, keeping every existing trainer on the old behaviour.
        int aiLevel = trainer.getAiLevel();
        BattleAI battleAI = aiLevel >= 0
            ? new ScoreBattleAI(aiLevel)
            : CobblemonTrainers.INSTANCE.getConfig().getCurrentAI();
        List<BattlePokemon> trainerTeam = trainer.rollBattleTeam();
        BattleActor trainerActor = trainerEntity == null ?
            new TrainerBattleActor(
                trainer.getName(), UUID.randomUUID(), trainerTeam, battleAI
            ) :
            new EntityBackerTrainerBattleActor(
                trainer.getName(), trainerEntity, UUID.randomUUID(), trainerTeam, battleAI
            );
    ...
    public static void startTrainerBattle(ServerPlayerEntity player, Trainer trainer, LivingEntity trainerEntity) {
        ...
        BattleFormat battleFormat = trainer.isDoubles() ?
            BattleFormat.Companion.getGEN_9_DOUBLES() : BattleFormat.Companion.getGEN_9_SINGLES();
        PokemonUtility.createTrainerBattle(player, trainer, trainerEntity, battleFormat)
            .ifErrored(error -> {
